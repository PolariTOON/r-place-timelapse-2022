...
const diffs = await readdir(fileURLToPath(`${root}/r-place-atlas-2022/web/_img/canvas/place30`));
diffs.unshift(`../place30ex/start.png`);
...
for (let i = 0, j = 0; i < diffCount; ++i) {
	...
	const diffImage = await loadImage(fileURLToPath(`${root}/r-place-atlas-2022/web/_img/canvas/place30/${diff}`));
	const base = diff.match(/^\d+_\d+\.png$/) != null ? diff.split("_")[1] : null;
	const baseImage = base != null ? await loadImage(fileURLToPath(`${root}/r-place-atlas-2022/web/_img/canvas/place30/${base}`)) : null;
	...
	context.drawImage(diffImage, x, y, 160, 90, 0, 0, 160 * 12, 90 * 12);
